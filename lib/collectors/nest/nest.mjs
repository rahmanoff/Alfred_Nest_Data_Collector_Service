...
async function _needsHeating(deviceData) {
  debug(`Check if home or on holiday, set heating accordingly`);

  const { ecoMode } = await this._masterEcoMode.call(this);
  ...
  if (!(await this._atHomeToday())) {
    ...
    if (deviceData.ecoMode === 'MANUAL_ECO') {
      const req = { params: { ecoMode: false } };
      await this._heating.call(this, req, null, null);
    } else {
      debug('Already in heat mode');
    }
  }
  return true;
}

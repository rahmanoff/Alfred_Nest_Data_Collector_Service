...
async function _masterEcoMode(req, res, next) {
  ...
  let masterEcoMode = false;
  let masterTemperature = 21;
  try {
    masterEcoMode = masterRecord[0].ecoMode;
    masterTemperature = masterRecord[0].temperature;
  } catch {
    debug('Master eco mode is missing from data');
  }

  if (typeof res !== 'undefined' && res !== null) {
    this._sendResponse(res, next, 200, { masterEcoMode, masterTemperature } );
  }
  return { masterEcoMode, masterTemperature };
}
...
async function _updateMasterEcoMode(req, res, next) {
  ...
  try {
    ...
    if (existingMasterEcoMode === null || existingMasterEcoMode === masterEcoMode) {
